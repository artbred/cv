...
from storage import save_download_by_label_id, save_query_position, save_user_request
...
@app.middleware("http")
async def request_params_middleware(request: Request, call_next):
    requested_path = request.url.path
    endpoint = None

    # Only API routes
    api_routes = filter(lambda r: "api" in getattr(r, "tags", []), app.router.routes)
    
    for route in api_routes:
        if route.path == requested_path:
            endpoint = requested_path.replace("/", "")
            break
    ...
    if endpoint:
        ...
        asyncio.create_task(save_user_request(endpoint, utm_params=utm_params))
    
    return await call_next(request)


@app.post("/score", tags=["api"])
